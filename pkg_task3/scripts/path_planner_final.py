...
class PathPlanner():
    ...
    def marker_error_publish(self):
        if(self.img_data[0]!=0.0 and self.check_marker):
            self.x_error=self.lat_to_x(self.x_to_lat_diff(self.img_data[0])-self.current_location[0])
            self.y_error=self.long_to_y(self.y_to_long_diff(self.img_data[1])-self.current_location[1])
            self.pub_error_X.publish(self.x_error)
            self.pub_error_Y.publish(self.y_error)
            self.pub_building_no.publish(self.building_id)
        else:
            self.pub_error_X.publish(0.0)
            self.pub_error_Y.publish(0.0)
