...
class PathPlanner():
    ...
    def __init__(self):
        ...
        # Destination checking for obstacle avoid
        self.take_destination = False
        # Formal counter for getting drone coordinates
        self.cnt = 0

        self.drone_co_ordinates = [0, 0, 0]             # Inital coordinates
        # For getting the x & y valuses from /marker_error
        self.img_data = [0, 0]
        # For switching in markr_find condition
        self.pose_cnt = 0
        # For switching in markr_find coordinates
        ...
        # Altitude addition upper coordinates
        self.altitude_up = 3
        # Altitude addition lower coordinates
        self.altitude_down = 1
        ...
        self.pub_error_X = rospy.Publisher(
            '/edrone/err_x_m', Float64, queue_size=1)
        self.pub_error_Y = rospy.Publisher(
            '/edrone/err_y_m', Float64, queue_size=1)
        self.pub_building_no = rospy.Publisher(
            '/edrone/curr_marker_id', Int8, queue_size=1)
        self.pub_marker_data = rospy.Publisher(
