...
class PathPlanner():
    ...
    def obstacle_avoid(self):
        ...
        self.current_location_xy = [self.lat_to_x(
            self.destination[1]), self.long_to_y(self.destination[2])]
        self.destination_xy = [self.lat_to_x(
            self.current_location[1]), self.long_to_y(self.current_location[2])]
        self.diff_xy = [self.destination_xy[0] - self.current_location_xy[0],
                self.destination_xy[1] - self.current_location_xy[1]]
        self.distance_xy = math.hypot(side[0], side[1])
        ...
        self.checkpoint.latitude = self.current_location[1] - self.y_to_long_diff(self.movement_in_plane[1])
